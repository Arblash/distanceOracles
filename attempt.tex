...
Let $G = (V, E)$ be an undirected unweighted graph with $|V| = n$ nodes and $|E| = m$ edges.
Let $\delta(u, v)$ denote the distance between nodes $u, v \in V$.
...
Given two nodes $u, v$ it can in constant time compute a distance estimate $\hat{\delta}(u, v)$ that satisfies $\delta(u, v) \leq \hat{\delta}(u, v) \leq \alpha\delta(u, v) + \beta$.
