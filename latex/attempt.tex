...
We will also compare the preprocessing memory and time.
}
\polishabstract {
...
W poniższej pracy porównamy na różnych zestawach grafów kilka wyroczni odległości (2,1) pod względem ich zajmowanej pamięci, szybkości działania i średniego błędu. Porównamy również czas i pamięć jaką zajmuje stworzenie wyroczni.
...
\begin{proof}
    ...
    as
    \newline
    1. Clearly every vertex $u$ from set $nei_{i-1}(v)$ have exactly one edge $(u, l)$
    such that $\delta(v, l)\leq i-1$
    \newline
    and 
    \newline
    2. Clearly there is no two distinct edges $(u, l),~(w, l)$ satisfying $u,w \in nei_{i-1}(v),$ $l\in~V$ 
...
\noindent\fbox{
    ...
    for $i \leftarrow 1$ to $k-1$
        ...
        ~~~~let $A_i$ be a uniform sample from $A_{i-1}$ of size $\ceil[\big]{n^{-1/k} * |A_{i-1}|}$
...
\noindent\fbox{
    ...
    ~~~~Let $S$ be a uniform sample from $X'$ of size $\ceil[\big]{p * |X|}$ or $X'$ if $\ceil[\big]{p * |X|} < |X'|$
...
\noindent\fbox{
    \begin{minipage}{\textwidth -14.1pt}
    algorithm $\mathbf{preproPatRo}(G)$

    let $R$ be a uniform sample from $V$ of size $\ceil[\big]{n^{-1/3} * n}$ 
    
    let $S$ be a uniform sample from $V$ of size $\ceil[\big]{n^{-2/3} * n}$
