...
\englishabstract{
    ...
    The $(3,0)$ distance oracle is the most precise oracle that have $\beta=0$ and takes subquadratic memory $-$ $O(n^{3/2})$.
    By using the additive error $-$~$\beta>0$ we are able to decrease $\alpha$ to $2$, while retaining subquadratic memory $-$ $(2,1)$ distance oracle takes only $O(n^{5/3})$ memory.
...
\begin{proof}
    ...
    as
    \newline
    1. Every vertex $u$ from set N$_{i-1}(v)$ have exactly one edge $(u, l)$
    such that $\delta(v, l)\leq i-1$
    \newline
    and 
    \newline
    2. There are no two distinct edges $(u, l),~(w, l)$ satisfying $u,w \in $N$_{i-1}(v),$ $l\in~V$ 
